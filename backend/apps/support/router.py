...
async def remove_ticket(
    ...
    res = await services.remove_ticket(user_id=payload["id"], ticket_id=ticket_id, db=db)
    if res == "success":
...
async def resolve_ticket_by_admin(
    ...
    res = await services.resolve_ticket_by_admin(user_id=payload["id"], ticket_id=ticket_id, db=db)
    if res == "success":
